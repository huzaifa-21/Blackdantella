  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get(
        `${config.BASE_URL}/api/products?limit=${limit}&page=${page}&category=${category}`
      );
        ...
        <div className="row">
          {products.map((product) => (
            <div key={product._id} className="col-6 col-sm-6 col-md-4 col-lg-3 mb-4">
              ...
              >
                <img src={`${config.BASE_URL}${product.colorVariants[0].images[0].url}`} alt={product.name} loading="lazy" />
